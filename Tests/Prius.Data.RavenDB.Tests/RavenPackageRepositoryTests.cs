...
public sealed class RavenPackageRepositoryTests : RavenTestDriver
{
    ...
    public async Task GetManifests_FiltersDependenciesBySpecificTfm_AccordingToNuspecMapperStructure()
    {
        ...
        var filteredDeps = targetPackageManifest.DeepGet(new MapPath("Dependencies".AsSpan())).AsMap();
    ...
    public async Task GetManifests_FillsDependenciesWithAny_WhenSpecificTfmIsMissing()
    {
        ...
        var filteredDeps = manifests[PackageId].AsMap().DeepGet(new MapPath("Dependencies".AsSpan())).AsMap();
    ...
    public async Task GetManifests_UsesInternalCache_OnSubsequentCalls()
    {
        ...
        Assert.Equal(PackageId, cachedManifest.DeepGet(new MapPath("Info/id".AsSpan())).AsString());
    }
    ...
    public async Task GetManifests_DoesNotMixCache_ForDifferentTfmsOfSamePackageVersion()
    {
        ...
        var deps1 = call1[PackageId].AsMap().DeepGet(new MapPath("Dependencies".AsSpan())).AsMap();
        ...
        var deps2 = call2[PackageId].AsMap().DeepGet(new MapPath("Dependencies".AsSpan())).AsMap();
    ...
    public async Task GetManifests_ReturnsManifestCorrectly_WhenPackageHasNoDependenciesLayer()
    {
        ...
        Assert.Equal(PackageId, manifest.DeepGet(new MapPath("Info/id".AsSpan())).ToString());
        Assert.True(manifest.DeepGet(new MapPath("Dependencies".AsSpan())).IsEmpty);
    }
