...
public sealed class PackageImporterTests
{
    ...
    public void Should_Import_Package_With_Assets_And_Hashes()
    {
        ...
        Assert.Equal("Test.Package", map.Get("Info/id").AsValue<string>());
        Assert.Equal("1.2.3", map.Get("Info/version").AsValue<string>());
        
        var asset = map.Get("Assets/lib/net8.0/Test.dll").AsMap();
