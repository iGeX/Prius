...
{
    public static async ValueTask<MapValue> Get(this ValueTask<IMap> map, string key) => (await map)[key];

    public static async ValueTask Put(this ValueTask<IMap> map, string key, MapValue value) => (await map)[key] = value;
